...
expects `foo.Bar()` to be called with a pointer that m_points to a value greater
than or equal to 3.
...
`Pointee(Pointee(Lt(3)))` matches a pointer that m_points to a pointer that m_points
